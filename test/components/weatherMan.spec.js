...
beforeEach(() => {
  fetch.mockResponse(JSON.stringify(cityDataMocked("Padua", "sunny", "8.5")), {
    status: 200
  })
...
test("responds to city change", () => {
  const registerCitySpy = sinon.spy(
    WeatherMan.prototype,
    "registerCitySearched"
  )
...
test("renders name and temperature of multiple cities added in the form of an unordered list", async () => {
  ...
  fetch.mockResponse(JSON.stringify(cityDataMocked("London", "misty", "-2")), {
    status: 200
  })
